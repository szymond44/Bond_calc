...
def simulate_paths(stacked_shocks, params_list, horizon, sim_number, dt=1/12):
    ...
    # Sprawdzamy ile mamy zmiennych makro (np. 2 dla CPI i WIBOR)
    num_variables = stacked_shocks.shape[0]
    
    # Tworzymy macierz 3D na wyniki: (zmienna, symulacja, czas)
    paths = np.zeros((num_variables, sim_number, horizon))
    
    # Krok 1: Wypełniamy wartości początkowe (miesiąc 0) dla każdej zmiennej
    for i in range(num_variables):
        paths[i, :, 0] = params_list[i]["start_value"]
        
    # Krok 2: Pętla po czasie (zaczynamy od 1, bo 0 to wartość startowa)
    for t in range(1, horizon):
        
        # Pętla po zmiennych w danym miesiącu (indeks 'i' to np. 0 dla CPI, 1 dla WIBOR)
        for i in range(num_variables):
